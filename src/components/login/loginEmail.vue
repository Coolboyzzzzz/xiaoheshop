<template>
  <div>
    ...
    >
      ...
      <el-form-item>
        <el-button size="medium" type="danger" @click="emailLogin">登录</el-button>
      </el-form-item>
      <div id="register">
        <!-- 分别传入不同的值，动态渲染drawer组件 -->
        <a @click="register(['SignIn1','注册'])">立即注册</a>
        <span style="border-left:1px solid rgb(178,178,178);width:1px;"></span>
        <a @click="register(['SignIn1','忘记密码'])">忘记密码</a>
      </div>
    </el-form>
    <!-- 导入注册组件 -->
    <el-drawer
...
export default {
  ...
  methods: {
    ...
    //   登录事件  用户名登录和邮箱登录
    async emailLogin() {
      this.$refs["SignIn1"].validate(async valid => {
        if (valid) {
          const { data: res } = await emailCodeLogin({
            email: this.ruleForm.email,
            code: this.ruleForm.checkEmail
          });
          if (res.status === 200) {
    ...
    sendEmail(formName) {
      ...
      this.$refs[formName].validateField("email", async err => {
        if (err) {
          ...
          const { data: res } = await sendEmailapi({
            email: this.ruleForm.email
          });
          //如果邮件发送失败
          if (res.code != 200) {
